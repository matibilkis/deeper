...
def real_training(run_id, number_betas=10, lr = 10**-2,
 states_wasted=10**4, min_ep=1, batch_size=10, epochs=10):
    ...
    buffer = ReplayBuffer(buffer_size=states_wasted)
    for episode in tqdm(range(states_wasted)):
        ...
        #
        # pt = train_net(episode,pt, network=q1, optimizer=optimizer, buffer=buffer, epochs=1, batch_size=10)
...
real_training(number_run, states_wasted=2*10**4)
