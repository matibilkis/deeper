...
def RDPG(special_name="", amplitude=0.4, dolinar_layers=2, number_phases=2, total_episodes = 10**3, buffer_size=500, batch_size=64, ep_guess=0.01, noise_displacement=0.5, lr_actor=0.01, lr_critic=0.001, tau=0.005):
    ...
    for episode in tqdm(range(total_episodes)):
        ...
        for layer in range(actor.dolinar_layers):
            ...
            beta = np.random.choice(np.arange(.1,1.1,.1))
    ...
    np.save("buffers/1L-stoch",buffer.sample(buffer.buffer_size) )
    return
...
if __name__ == "__main__":
    ...
    for batch_size in [8.]:
        ...
        name_run = RDPG(amplitude=amplitude, total_episodes=10**4, dolinar_layers=dolinar_layers, noise_displacement=noise_displacement, tau=tau, buffer_size=buffer_size, batch_size=batch_size, lr_critic=lr_critic, lr_actor=lr_actor, ep_guess=ep_guess)
